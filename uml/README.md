...
By now, you have seen many examples of pretty arcane code that make even simple programs hard to follow.
...
In this course, we will discuss class diagrams. There's another popular type of UML, known as sequence diagrams, but we won't have time to get to it in class. 
...
There are many tools in the market for preparing UML diagrams. There are specialized enterprise-quality tools such as "Enterprise Architect" and IBM Rational, and free tools such as ArgoUML, as well as general modeling tools such as Lucidchart and Microsoft Visio. Many tools provide the capability to reverse-engineer code, as well as generate code from models. Usually the choice of tool will be dictated in your shop.
...
Aggregation and composition are subsets of association; in fact, the generated code would be identical. Whereas association just indicates that class A has a field of type B, aggregation indicates that class A has a field of type B, but that B can exist independently of A. 
...
A building has rooms and a superintendent. If you take away the superindendent, you still have a building (for a while!). That is aggregation. But if you take away the rooms, the building ceases to exist. That is composition. And both are associations.

> Note the direction of the arrows. In assocation, the arrow head is on the dependent class, whereas with aggregation and composition, the rhombus is on the containing class.
