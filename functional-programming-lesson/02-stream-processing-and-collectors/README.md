...
In this lesson, we'll go into more detail on how to create streams, the different intermediate and terminal stream operations, and collectors.

The first demo will show you the different ways to create a stream.
...
But you don't need to start with a collection to create a stream. You can create a stream from an array:
...
In the following sections and demos, we'll see why and when you'd use each type.
...
The last example for this demo will show you how improperly ordering your intermediate operations can cause unnecessary processing.
...
I would say pick the filter that produces the least number of results. So which filter would that be?
...
- `max`: terminal operation that returns the maximum element based on a comparison.
...
In the previous module, we talked briefly about collectors. Again, collectors are used to collect data out of a stream and put it into a data structure. In this section, we'll look at other data structures we can output to. They are:  
...
`groupingBy` is an advanced partitioning that allows you to create custom groupings. The example below will group all the persons based on the first letter of their name. The lambda expression passed to the `groupingBy` method tells the collector what to use for the `Map` key. Notice that the value of the `Map` is a list of **one or more `Person` instances**.
