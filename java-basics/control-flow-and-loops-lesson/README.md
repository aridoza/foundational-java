...
The way the control flow different components are implemented is what varies from language to language. So let's see how it's done in Java.
...
The `&&` and `||` operators use short-circuit logic, which means that whether they will execute their second operand is dependent on the first.
...
Well, let's do another example to show you which and why:
...
In just about all programming languages, loop-ing exists.  A loop is a statement or block of code that will continue to execute while or until a condition exists.
...
Notice the placement of the comma and semicolons, and let's take a look at what each of the parts do:
